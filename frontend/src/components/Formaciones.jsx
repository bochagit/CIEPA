...
export default function Formaciones() {
    ...
    React.useEffect(() => {
        const fetchFormaciones = async () => {
            try {
                setLoading(true)
                const response = await eventService.getEventsByType('formacion', 20)
                setEvents(response.events || [])
                setError('')
            } catch(err) {
                setError('Error al cargar ciclo de formaciones')
                console.error(err)
            } finally {
                setLoading(false)
            }
        ...
        {loading ? (
            ...
            <Box sx={{ textAlign: 'center', py: 8 }}>
                <Typography variant="h6" color="text.secondary" gutterBottom>
                    No hay ciclo de formaciones disponibles
                </Typography>
                <Typography variant="body2" color="text.secondary">
